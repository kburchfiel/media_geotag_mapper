...
def retrieve_clip_locations(df_clips):
    ...
    for i in tqdm(range(len(df_clips))):
        try:
            ...
            video_location_list.append(metadata['format']['tags']['location'])
...
path_color = '#3388ff', path_weight = 3, tiles = 'Stamen Terrain'):
    ...
    timestamp_column = locations_to_map.columns.get_loc('modified_time')
...
Using kilometers as the distance measure.")
    ...
    # for each year.
    df['year'] = df['modified_time'].dt.to_period('Y')
    # See:
